...
import { ChevronRight, ChevronDown, Search, BookOpen, Users, Bug, Plus } from "lucide-react";
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      <aside
        className={cn(
          ...
          "fixed lg:sticky inset-y-16 lg:top-16 left-0 z-40 transition-transform h-[calc(100vh-4rem)]",
        ...
        <article className="max-w-3xl mx-auto px-6 py-10">
          ...
          <div className="prose prose-invert prose-sm max-w-none">
            {content.content.split("\n\n").map((para, i) => {
              ...
              if (para.startsWith("```")) {
                ...
                return (
                  <pre key={i} className="bg-secondary rounded-lg p-4 text-sm text-foreground overflow-x-auto my-4">
                    <code>{code}</code>
                  </pre>
                );
              }
          ...
          <div className="mt-12 pt-8 border-t border-border/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { title: "Prompt Engineering Course", desc: "Complete course on mastering prompts for any LLM." },
                { title: "Advanced Techniques Workshop", desc: "Deep dive into CoT, ToT, and meta-prompting." },
              ].map((card) => (
                <div
                  key={card.title}
